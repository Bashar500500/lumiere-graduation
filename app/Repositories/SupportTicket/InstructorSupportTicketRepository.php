...
class InstructorSupportTicketRepository extends BaseRepository implements SupportTicketRepositoryInterface
{
    ...
    {
        return (object) $this->model->where('user_id', $data['userId'])
            ->with('user')
            ->latest('created_at')
            ->simplePaginate(
                $dto->pageSize,
